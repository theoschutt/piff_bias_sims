...
class DC2_SEDCatalog(Catalog):
    ...
    def getSED(self, index):
        ...
        sed_bins = [_q for _q in sed_array.dtype.names if re.match(r"sed_\d+_\d+$", _q)] # change for no host extinction
        ...
        ) * 4.4659e13 * 1e7 / _rubin_area  # convert from 4.4659e13 W/Hz to erg/Hz/cm^2/s (i.e., nu) # this doesn't work
        ...
        sed = sed.withMagnitude(float(sed_array["mag_true_r_lsst"]), bps_zp[2])  # use the provided magnitudes to set overall normalization
